...
if __name__ == "__main__":
    ...
    blossomlibrary = Library(
        ["it ends with us", "Verity", "atomic habits", "the sweetest oblivion", "a goodgirl's guide to murder", "the cruel prince"])
    student = Student()
    track = []
    ...
    while (True):
        ...
        try:
            ...
            if usr_response == 1:  # listing
                blossomlibrary.displayAvailableBooks()
            elif usr_response == 2:  # borrow
                blossomlibrary.borrowBook(
                    input("Enter your name: "), student.requestBook())
            elif usr_response == 3:  # return
                blossomlibrary.returnBook(student.returnBook())
            elif usr_response == 4:  # donate
                blossomlibrary.donateBook(student.donateBook())
